...
data {
  ...
  int <lower = 0> n_basis2;							// no. spline basis functions for variable 2
  ...
  matrix[n_basis2, n_data] B2;						// Values of var 2 basis functions for each function at each data point
  vector[n_basis1] norms1;							// Areas under curves of var 1 basis functions
  vector[n_basis2] norms2;							// Areas under curves of var 2 basis functions
...
parameters {
  ...
  row_vector[n_basis2] a_raw2;		// Coefficients of var 2 basis functions
}
...
transformed parameters {
  ...
  a2[1]=a_raw2[1];
  for (i in 2:n_basis2)
    a2[i] = a2[i-1] + a_raw2[i];
	
  // Subtract area under curve
  scale_fac=-dot_product(a2, norms2)/sum(norms2);
  a2=a2+scale_fac;
}
...
model {
  ...
  vector[n_data] y;   
  y = alpha[Strain] + to_vector(a1*B1) + to_vector(a2*B2);
